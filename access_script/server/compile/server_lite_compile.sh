...
#WORKSPACE=/usr1/jenkins_home/workspace
...
function download_source() {
    cd ${WORKSPACE}
    download_source_from_gitee ${server_repo} ${giteeTargetBranch} openGauss-lite
}

function merge_source_code() {
    cd ${WORKSPACE}/openGauss-lite
    git rev-parse --is-inside-work-tree
    git config remote.origin.url ${server_repo}
...
function lite_complie() {
    cd ${WORKSPACE}/openGauss-lite/build/script
    chmod +x cmake_package_mini.sh
    sh cmake_package_mini.sh -3rd ${WORKSPACE}/openGauss-third_party_binarylibs -pkg server

    if [ $? -ne 0 ]; then
        echo "ERROR: server lit make compile failed!"
        exit 1
    fi
    echo "Success: server lite make compile success !!!"
}

function set_env() {
    export CMAKEROOT=/opt/install/cmake-3.28.4-linux-aarch64
    export LD_LIBRARY_PATH=$CMAKEROOT/lib:$LD_LIBRARY_PATH
