...
portal_bak_dir=${workspace}/../portal_bak
...
function down_soure_from_gitee() {
    ...
    timeout 300 git clone ${repo} -b ${branch} "${workspace}/${target_dir}"
    if [[ $? == 0 ]] ; then
        flag=1
        break
    fi
...
function mv_pkg() {
    if [[ ! -d ${portal_bak_dir} ]]; then
        mkdir -p ${portal_bak_dir}
    fi
    cd ${portal_bak_dir}
    nums=$(ls -l |grep "^-"|wc -l)
    if [[ $nums = 0 ]]; then
...
function mv_config(){
    ...
    sh main.sh "CentOS7_x86_64" ${workspace}/openGauss-migration-portal/portal
...
function package() {
    ...
    sed -i "s/system.name=Centos7.6/system.name=${os_name}/g" portal/config/toolspath.properties
